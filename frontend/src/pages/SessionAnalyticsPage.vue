...
async function deleteSession() {
  ...
  try {
    const res = await fetch(apiUrl(`/api/sessions/${route.params.sessionId}/permanent`), {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${auth.token}` }
    });
    ...
    alert(err.message || t('errors.failedToDeleteSession'));
  }
}
      ...
      <template v-else-if="sessionData">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          ...
          <div class="flex items-center gap-2 flex-wrap">
            ...
            <PixelButton variant="outline" @click="deleteSession">
              {{ t('analytics.deleteSession') }}
            </PixelButton>
          </div>
        </div>
